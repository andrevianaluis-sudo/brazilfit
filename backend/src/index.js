...
app.get('/api/fix-neil-dupes', (req, res) => { try { const { getDb } = require('./db/database'); const db = getDb(); const user = db.prepare('SELECT id FROM users WHERE username=?').get('neil_crawley'); const client = db.prepare('SELECT id FROM clients WHERE user_id=?').get(user.id); const sessions = db.prepare('SELECT id, scheduled_date, scheduled_time FROM sessions WHERE client_id=? AND status=? ORDER BY id ASC').all(client.id, 'upcoming'); const seen = {}; const toDelete = []; for(const s of sessions){ const key = s.scheduled_date + '_' + s.scheduled_time; if(seen[key]){ toDelete.push(s.id); } else { seen[key] = true; } } for(const id of toDelete){ db.prepare('DELETE FROM sessions WHERE id=?').run(id); } res.json({deleted: toDelete.length, done:true}); } catch(e){res.status(500).json({error:e.message});} });
...
app.get('/api/fix-neil-times', (req, res) => { try { const { getDb } = require('./db/database'); const db = getDb(); const user = db.prepare('SELECT id FROM users WHERE username=?').get('neil_crawley'); const client = db.prepare('SELECT id FROM clients WHERE user_id=?').get(user.id); const r1 = db.prepare("DELETE FROM sessions WHERE client_id=? AND scheduled_time='11:00' AND status='upcoming'").run(client.id); const r2 = db.prepare("DELETE FROM sessions WHERE client_id=? AND scheduled_time='11:00:00' AND status='upcoming'").run(client.id); res.json({deleted: r1.changes + r2.changes, done:true}); } catch(e){res.status(500).json({error:e.message});} });
app.get('*', (req, res) => {
