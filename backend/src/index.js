...
app.get('/api/update-sessions2', (req, res) => { try { const { getDb } = require('./db/database'); const db = getDb(); const updates = [ {u:'andy_devlin', sessions:5}, {u:'clare_moody', sessions:9}, {u:'craig_burton', sessions:6}, {u:'laura_graham', sessions:5}, {u:'james_graham', sessions:4}, {u:'jaquetta_devine', sessions:8}, {u:'michelle_pegg', sessions:8}, {u:'neil_crawley', sessions:4}, {u:'sue_crawley', sessions:0}, {u:'vivien', sessions:5}, {u:'sharon_langridge', sessions:6}, ]; let updated=0; for(const c of updates){ const user=db.prepare('SELECT id FROM users WHERE username=?').get(c.u); if(!user) continue; const client=db.prepare('SELECT id FROM clients WHERE user_id=?').get(user.id); if(!client) continue; db.prepare('UPDATE clients SET sessions_used=? WHERE id=?').run(c.sessions,client.id); updated++; } res.json({updated}); } catch(e){res.status(500).json({error:e.message});} });
app.get('/api/update-sessions3', (req, res) => { try { const { getDb } = require('./db/database'); const db = getDb(); const updates = [ {u:'noah_crosse', sessions:7}, {u:'puja_aggarwall', sessions:5} ]; let updated=0; for(const c of updates){ const user=db.prepare('SELECT id FROM users WHERE username=?').get(c.u); if(!user) continue; const client=db.prepare('SELECT id FROM clients WHERE user_id=?').get(user.id); if(!client) continue; db.prepare('UPDATE clients SET sessions_used=? WHERE id=?').run(c.sessions,client.id); updated++; } res.json({updated}); } catch(e){res.status(500).json({error:e.message});} });
app.get('/api/update-sessions4', (req, res) => { try { const { getDb } = require('./db/database'); const db = getDb(); const updates = [ {u:'noah_crosse', sessions:7}, {u:'puja_aggarwall', sessions:5} ]; let updated=0; for(const c of updates){ const user=db.prepare('SELECT id FROM users WHERE username=?').get(c.u); if(!user) continue; const client=db.prepare('SELECT id FROM clients WHERE user_id=?').get(user.id); if(!client) continue; db.prepare('UPDATE clients SET sessions_used=? WHERE id=?').run(c.sessions,client.id); const block=db.prepare('SELECT id FROM blocks WHERE client_id=? AND is_current=1').get(client.id); if(block){ db.prepare('UPDATE blocks SET sessions_attended=? WHERE id=?').run(c.sessions,block.id); } updated++; } res.json({updated, done:true}); } catch(e){res.status(500).json({error:e.message});} });
app.get('/api/fix-all-dupes', (req, res) => { try { const { getDb } = require('./db/database'); const db = getDb(); const sessions = db.prepare('SELECT id, client_id, scheduled_date, scheduled_time, status FROM sessions WHERE status=? ORDER BY id ASC').all('upcoming'); const seen = {}; const toDelete = []; for(const s of sessions){ const key = s.client_id + '_' + s.scheduled_date + '_' + s.scheduled_time; if(seen[key]){ toDelete.push(s.id); } else { seen[key] = true; } } for(const id of toDelete){ db.prepare('DELETE FROM sessions WHERE id=?').run(id); } res.json({deleted: toDelete.length, done:true}); } catch(e){res.status(500).json({error:e.message});} });
app.get('/api/neil-to-3', (req, res) => { try { const { getDb } = require('./db/database'); const db = getDb(); const user = db.prepare('SELECT id FROM users WHERE username=?').get('neil_crawley'); const client = db.prepare('SELECT id FROM clients WHERE user_id=?').get(user.id); db.prepare('UPDATE clients SET sessions_used=3 WHERE id=?').run(client.id); const block = db.prepare('SELECT id FROM blocks WHERE client_id=? AND is_current=1').get(client.id); if(block) db.prepare('UPDATE blocks SET sessions_attended=3 WHERE id=?').run(block.id); res.json({done:true}); } catch(e){res.status(500).json({error:e.message});} });
app.get('/api/fix-neil-fri', (req, res) => { try { const { getDb } = require('./db/database'); const db = getDb(); const user = db.prepare('SELECT id FROM users WHERE username=?').get('neil_crawley'); const client = db.prepare('SELECT id FROM clients WHERE user_id=?').get(user.id); const r = db.prepare("DELETE FROM sessions WHERE client_id=? AND scheduled_date='2026-05-29' AND scheduled_time IN ('08:00','08:00:00') AND status='upcoming'").run(client.id); res.json({deleted:r.changes, done:true}); } catch(e){res.status(500).json({error:e.message});} });
app.get('/api/add-neil-thisweek', (req, res) => { try { const { getDb } = require('./db/database'); const db = getDb(); const user = db.prepare('SELECT id FROM users WHERE username=?').get('neil_crawley'); const client = db.prepare('SELECT id FROM clients WHERE user_id=?').get(user.id); const existing1 = db.prepare("SELECT id FROM sessions WHERE client_id=? AND scheduled_date='2026-05-26'").get(client.id); const existing2 = db.prepare("SELECT id FROM sessions WHERE client_id=? AND scheduled_date='2026-05-29'").get(client.id); if(!existing1) db.prepare("INSERT INTO sessions (client_id, scheduled_date, scheduled_time, status) VALUES (?, '2026-05-26', '10:00', 'upcoming')").run(client.id); if(!existing2) db.prepare("INSERT INTO sessions (client_id, scheduled_date, scheduled_time, status) VALUES (?, '2026-05-29', '09:00', 'upcoming')").run(client.id); res.json({done:true}); } catch(e){res.status(500).json({error:e.message});} });
app.get('/api/fix-jaquetta', (req, res) => { try { const { getDb } = require('./db/database'); const db = getDb(); db.prepare("UPDATE users SET name = 'Jacquetta Devine' WHERE username = 'jaquetta_devine'").run(); res.json({ done: true }); } catch(e) { res.status(500).json({ error: e.message }); } });
app.get('/api/fix-jacquetta-all', (req, res) => { try { const { getDb } = require('./db/database'); const db = getDb(); db.prepare("UPDATE users SET name = 'Jacquetta Devine', username = 'jacquetta_devine' WHERE username = 'jaquetta_devine'").run(); res.json({ done: true }); } catch(e) { res.status(500).json({ error: e.message }); } });
