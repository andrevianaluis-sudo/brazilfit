...
app.get('/api/create-charlotte', (req, res) => { try { const { getDb } = require('./db/database'); const bcrypt = require('bcryptjs'); const db = getDb(); const existing = db.prepare("SELECT id FROM users WHERE username = 'charlotte.blyth'").get(); if (existing) return res.json({ error: 'Already exists', id: existing.id }); const hash = bcrypt.hashSync('BrazilFit2026!', 10); const result = db.prepare("INSERT INTO users (email, username, password_hash, role, name, is_active) VALUES ('charlotteblyth@hotmail.com', 'charlotte.blyth', ?, 'client', 'Charlotte Blyth', 1)").run(hash); const userId = result.lastInsertRowid; const client = db.prepare("SELECT id FROM clients WHERE user_id IN (SELECT id FROM users WHERE email = 'charlotteblyth@hotmail.com') LIMIT 1").get(); if (client) { db.prepare('UPDATE clients SET user_id = ? WHERE id = ?').run(userId, client.id); res.json({ done: true, linked: true, clientId: client.id }); } else { db.prepare('INSERT INTO clients (user_id, client_type, block_price, pt_id) VALUES (?, ?F2F?, 400, 1)').run(userId); res.json({ done: true, linked: false }); } } catch(e) { res.status(500).json({ error: e.message }); } });
app.get('/api/fix-charlotte', (req, res) => { try { const { getDb } = require('./db/database'); const db = getDb(); const user = db.prepare("SELECT id, username FROM users WHERE email = 'charlotteblyth@hotmail.com'").get(); if (!user) return res.json({ error: 'No user found' }); db.prepare("UPDATE users SET username = 'charlotte.blyth', name = 'Charlotte Blyth' WHERE id = ?").run(user.id); const client = db.prepare('SELECT id FROM clients WHERE user_id = ?').get(user.id); res.json({ done: true, oldUsername: user.username, clientLinked: !!client }); } catch(e) { res.status(500).json({ error: e.message }); } });
