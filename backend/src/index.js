...
app.get('/api/fix-vivien-username', (req, res) => { try { const { getDb } = require('./db/database'); const db = getDb(); db.prepare("UPDATE users SET username = 'vivien_twaddle' WHERE username = 'vivien'").run(); res.json({ done: true }); } catch(e) { res.status(500).json({ error: e.message }); } });
app.get('/api/fix-vivien-sessions', (req, res) => { try { const { getDb } = require('./db/database'); const db = getDb(); const user = db.prepare('SELECT id FROM users WHERE username = ?').get('vivien_twaddle'); const client = db.prepare('SELECT id FROM clients WHERE user_id = ?').get(user.id); db.prepare('UPDATE clients SET sessions_used = 6 WHERE id = ?').run(client.id); const block = db.prepare('SELECT id FROM blocks WHERE client_id = ? AND is_current = 1').get(client.id); if(block) db.prepare('UPDATE blocks SET sessions_attended = 6 WHERE id = ?').run(block.id); res.json({ done: true }); } catch(e) { res.status(500).json({ error: e.message }); } });
app.get('/api/fix-neil', (req, res) => { try { const { getDb } = require('./db/database'); const db = getDb(); const user = db.prepare('SELECT id FROM users WHERE username = ?').get('neil_crawley'); const client = db.prepare('SELECT id FROM clients WHERE user_id = ?').get(user.id); db.prepare('UPDATE clients SET sessions_used = 4 WHERE id = ?').run(client.id); const block = db.prepare('SELECT id FROM blocks WHERE client_id = ? AND is_current = 1').get(client.id); if(block) db.prepare('UPDATE blocks SET sessions_attended = 4 WHERE id = ?').run(block.id); res.json({ done: true }); } catch(e) { res.status(500).json({ error: e.message }); } });
app.get('/api/fix-all-sessions', (req, res) => { try { const { getDb } = require('./db/database'); const db = getDb(); const updates = [ {u:'charlotte.blyth',s:1}, {u:'christine_constant',s:6}, {u:'filomena_saulino',s:7}, {u:'hilary_clixby',s:3}, {u:'laura_graham',s:5}, {u:'louise_george',s:5}, {u:'lucy_clarke',s:7}, {u:'margaret_piggot',s:2}, {u:'michelle_pegg',s:8}, {u:'noah_crosse',s:7}, {u:'puja_aggarwall',s:5}, {u:'sharon_langridge',s:6} ]; let updated=0; for(const c of updates){ const user=db.prepare('SELECT id FROM users WHERE username=?').get(c.u); if(!user) continue; const client=db.prepare('SELECT id FROM clients WHERE user_id=?').get(user.id); if(!client) continue; db.prepare('UPDATE clients SET sessions_used=? WHERE id=?').run(c.s,client.id); const block=db.prepare('SELECT id FROM blocks WHERE client_id=? AND is_current=1').get(client.id); if(block) db.prepare('UPDATE blocks SET sessions_attended=? WHERE id=?').run(c.s,block.id); updated++; } res.json({updated, done:true}); } catch(e){res.status(500).json({error:e.message});} });
app.get('*', (req, res) => {
