...
export default function ClientLayout() {
  ...
  const fetchNotifications = () => {
    api.get('/messages/client-notifications')
      .then(r => { setUnreadCount(r.data.unreadCount || 0); setNotifications(r.data.notifications || []); })
      .catch(() => {});
  };
  ...
  const handleBellClick = () => {
    ...
    if (!showNotifs && unreadCount > 0) {
      api.put('/messages/client-notifications/read-all')
                ...
                <div style={{ position: 'fixed', top: '68px', right: '12px', width: 'min(340px, calc(100vw - 24px))', backgroundColor: '#1a1a1a', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', boxShadow: '0 20px 60px rgba(0,0,0,0.8)', zIndex: 999, overflow: 'hidden' }}>
                  ...
                  <div style={{ maxHeight: '380px', overflowY: 'auto' }}>
                    {notifications.length === 0 ? (
                      ...
                      <div
                        key={n.id}
                        onClick={() => { if (n.type === 'message') { navigate('/client/messages'); setShowNotifs(false); } else if (n.type === 'session' || n.type === 'reinstate' || n.type === 'cancellation') { navigate('/client/sessions'); setShowNotifs(false); } }}
                        style={{ padding: '12px 16px', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', gap: '12px', alignItems: 'flex-start', cursor: 'pointer', backgroundColor: n.is_read ? 'transparent' : 'rgba(255,107,43,0.05)', transition: 'background 0.15s' }}
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#141414', display: 'flex' }}>
      ...
      <div className="md:ml-60" style={{ flex: 1, display: 'flex', flexDirection: 'column', transition: 'margin 0.25s ease' }}>
        ...
        {/* Content â€” KEY FIX: background is #141414 not #141414 */}
        <main style={{ flex: 1, overflowY: 'auto', backgroundColor: '#0f0f0f' }}>
          <Outlet />
        </main>
      </div>
