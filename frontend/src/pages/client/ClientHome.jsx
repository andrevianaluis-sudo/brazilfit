    ...
    <div style={{background:'#111',minHeight:'100vh',paddingBottom:'6rem',fontFamily:"'DM Sans',system-ui,sans-serif"}}>
      <div style={{maxWidth:'800px',margin:'0 auto',padding:'2rem 1.25rem'}}>
        <div style={{marginBottom:'1.5rem'}}>
          ...
          <h1 style={{fontFamily:"'Clash Display',system-ui",fontSize:'2.5rem',fontWeight:800,color:'#fff',letterSpacing:'-0.04em',lineHeight:1.1,margin:'0 0 0.3rem'}}>{name}</h1>
        ...
        <div onClick={()=>navigate('/client/sessions')} style={{background:'linear-gradient(135deg,#FF6B2B,#FFD600)',borderRadius:'14px',padding:'1.25rem 1.5rem',marginBottom:'1rem',cursor:'pointer'}}>
          ...
          <p style={{fontFamily:"'Clash Display',system-ui",fontSize:'1.1rem',fontWeight:700,color:'#000',margin:0}}>{sessions?.upcoming?.[0]?.title||'No session booked yet'}</p>
        </div>
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'8px',marginBottom:'1rem'}}>
          {[{v:rem,l:'Sessions Left',c:sc},{v:5,l:'Week Streak',c:'#FF6B2B'},{v:used,l:'Completed',c:'#FFD600'}].map((s,i)=>(
            <div key={i} style={{...C,textAlign:'center'}}>
              <p style={{fontFamily:"'Clash Display',system-ui",fontSize:'2.2rem',fontWeight:800,color:s.c,letterSpacing:'-0.03em',lineHeight:1,margin:'0 0 0.4rem'}}>{s.v}</p>
              <p style={{fontSize:'0.6rem',fontWeight:700,letterSpacing:'0.1em',color:'#707070',textTransform:'uppercase',margin:0}}>{s.l}</p>
            </div>
          ))}
        </div>
        <div style={{...C,marginBottom:'1rem'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'0.75rem'}}>
            ...
            <p style={{fontFamily:"'Clash Display',system-ui",fontSize:'1.6rem',fontWeight:800,color:'#4CAF50',margin:0}}>{Math.round(pct)}%</p>
          </div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px',marginBottom:'1rem'}}>
          {LINKS.map((lk,i)=>{ const Icon=lk.ic; return (
            <div key={i} onClick={()=>navigate(lk.to)} style={{...C,cursor:'pointer',display:'flex',flexDirection:'column',gap:'0.6rem'}}>
              ...
              <div>
                <p style={{fontSize:'0.875rem',fontWeight:700,color:'#fff',margin:'0 0 2px'}}>{lk.label}</p>
